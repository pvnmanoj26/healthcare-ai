...
# ─────────────────────────────────────────────
# NEW ROUTING: LANGGRAPH WORKFLOW (HITL WEB UI)
# ─────────────────────────────────────────────

@app.route("/langgraph", methods=["GET"])
def langgraph_page():
    from agents.langgraph_workflow import app as langgraph_app
    thread_id = session.get("langgraph_thread_id")
    state_values = None
    ...
    if thread_id:
        try:
            ...
            print(f"Error fetching LangGraph state: {e}")
            session.pop("langgraph_thread_id", None)
            thread_id = None
...
@app.route("/langgraph/start", methods=["POST"])
def langgraph_start():
    from agents.langgraph_workflow import app as langgraph_app
    note = request.form.get("clinical_note", "").strip()
    if not note:
        return redirect(url_for("langgraph_page"))
...
@app.route("/langgraph/approve", methods=["POST"])
def langgraph_approve():
    from agents.langgraph_workflow import app as langgraph_app
    thread_id = session.get("langgraph_thread_id")
    if not thread_id:
        return redirect(url_for("langgraph_page"))
    ...
    try:
        state = langgraph_app.get_state(config)
        all_gaps = state.values.get("final_draft_gaps", [])
        
        approved_indices_str = request.form.getlist("approved_gaps")
        approved_gaps = []
        for idx_str in approved_indices_str:
            try:
                idx = int(idx_str)
                if 0 <= idx < len(all_gaps):
                    approved_gaps.append(all_gaps[idx])
            except ValueError:
                pass
                
        langgraph_app.update_state(config, {"approved_gaps": approved_gaps}, as_node="gap_prioritizer")
        langgraph_app.invoke(None, config)
        
    except Exception as e:
        print(f"Error resuming LangGraph workflow: {e}")
        import traceback
        traceback.print_exc()
        
    return redirect(url_for("langgraph_page"))

@app.route("/langgraph/clear")
def langgraph_clear():
    session.pop("langgraph_thread_id", None)
    return redirect(url_for("langgraph_page"))
...
<body>
  ...
  <div class="tabs">
    ...
    <div class="tab {% if active_tab == 'langgraph' %}active{% endif %}" id="tab-langgraph" onclick="switchTab('langgraph')">⚡ Care Gap Workflow</div>
  ...
  <!-- ── LANGGRAPH CARE GAP WORKFLOW ────────────────────────── -->
  <div class="form-panel {% if active_tab == 'langgraph' %}active{% endif %}" id="panel-langgraph">
    {% if not langgraph_thread_id %}
    <form method="post" action="/langgraph/start">
      <p style="color:#4a5568;">
        Start a stateful Care Gap Closure Workflow. This triggers a multi-stage LangGraph workflow including 
        <b>summarization</b>, <b>risk triage</b>, <b>parallel condition protocol checks</b>, 
        and pauses for your <b>clinical approval</b> before generating the final action plan.
      </p>
      <textarea name="clinical_note" placeholder="Paste patient note or discharge summary here...">{{ note_text or '' }}</textarea>
      <br><br>
      <button type="submit" class="btn btn-purple">⚡ Start Stateful Workflow</button>
    </form>
    {% else %}
      {% if langgraph_state %}
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
          <h2 style="margin:0;">⚡ Workflow Status: <span style="color:#553c9a;">{{ langgraph_state.status | upper }}</span></h2>
          <a href="/langgraph/clear" class="btn btn-red btn-sm">🗑️ Clear / Start New</a>
        </div>
        
        <!-- STEP 1 & 2: Clinical Summary & Risk Triage -->
        <div class="card">
          <h3>📋 Structured Summary</h3>
          <div class="section"><span class="label">Risk Level:</span> 
            <span class="tag {% if langgraph_state.risk_level == 'HIGH' %}tag-high{% elif langgraph_state.risk_level == 'MEDIUM' %}tag-medium{% else %}tag-low{% endif %}">
              {{ langgraph_state.risk_level }} RISK
            </span>
          </div>
          <div class="section"><span class="label">Primary Diagnosis:</span> {{ langgraph_state.summary.primary_diagnosis }}</div>
          <div class="section"><span class="label">Procedure:</span> {{ langgraph_state.summary.procedure or '—' }}</div>
          <div class="section">
            <span class="label">Comorbidities:</span>
            <ul>{% for c in langgraph_state.summary.comorbidities %}<li>{{ c }}</li>{% endfor %}</ul>
          </div>
          <div class="section">
            <span class="label">Medications:</span>
            <ul>{% for m in langgraph_state.summary.medications %}<li>{{ m }}</li>{% endfor %}</ul>
          </div>
        </div>

        {% if langgraph_state.status == 'pending_approval' %}
        <!-- STEP 3: Clinician Approval Checklist (HITL Gate) -->
        <div class="card card-purple">
          <h3>🩺 Clinician Human-in-the-Loop Sign-off</h3>
          <p style="color:#4a5568; font-size:13px;">
            The LangGraph execution has **paused** at the approval gate. Check the care gaps you wish to approve, then submit to resume execution.
          </p>
          <form method="post" action="/langgraph/approve">
            {% for gap in langgraph_state.final_draft_gaps %}
            <div style="margin:12px 0; padding:12px; background:#faf5ff; border-radius:6px; border-left:3px solid #553c9a; display:flex; align-items:flex-start; gap:12px;">
              <input type="checkbox" name="approved_gaps" value="{{ loop.index0 }}" checked style="width:20px; height:20px; margin-top:3px;">
              <div>
                <span class="tag {% if gap.priority == 'HIGH' %}tag-high{% elif gap.priority == 'MEDIUM' %}tag-medium{% else %}tag-low{% endif %}">
                  {{ gap.priority }} Priority
                </span>
                <div style="margin-top:4px;"><b>Gap:</b> {{ gap.gap }}</div>
                <div><b>Guideline:</b> <i>{{ gap.guideline }}</i></div>
                <div style="color:#2d3748; margin-top:2px;"><b>Recommendation:</b> {{ gap.recommendation }}</div>
              </div>
            </div>
            {% else %}
            <p style="color:#718096; font-style:italic;">No care gaps were identified for this patient record.</p>
            {% endfor %}
            <br>
            <button type="submit" class="btn btn-green">✅ Approve &amp; Generate Care Plan</button>
          </form>
        </div>
        {% elif langgraph_state.status == 'completed' %}
        <!-- STEP 4: Action Plan & Finalized Gaps -->
        <div class="card card-purple">
          <h3>✅ Clinician Approved Care Gaps</h3>
          {% for gap in langgraph_state.approved_gaps %}
            <div style="margin:10px 0; padding:8px 12px; background:#f0fff4; border-radius:6px; border-left:3px solid #276749; font-size:13px;">
              <b>Gap:</b> {{ gap.gap }} ({{ gap.priority }} Priority)
            </div>
          {% else %}
            <p style="color:#718096; font-style:italic;">No care gaps were approved.</p>
          {% endfor %}
        </div>
        
        <div class="card card-purple">
          <h3>📝 Actionable Care Plan</h3>
          <div class="note-text" style="background:#edf2f7; padding:16px; border-radius:6px; white-space:pre-wrap; font-family:sans-serif; font-size:14px; line-height:1.6;">
            {{ langgraph_state.action_plan }}
          </div>
          <br>
          <p style="color:#276749; font-weight:600; font-size:13px;">
            🎉 Workflow complete! Data and action plan saved to BigQuery.
          </p>
        </div>
        {% endif %}
      {% else %}
        <p style="color:#c53030;">Failed to retrieve active workflow state. Please try again.</p>
        <a href="/langgraph/clear" class="btn btn-red btn-sm">🗑️ Reset</a>
      {% endif %}
    {% endif %}
  </div>
